          ...
          container('jnlp') {
            ...
            withSonarQubeEnv('Sonarqube-server') {
              sh '''
                ...
                if [[ $BRANCH_NAME = pre* ]] || [[ $BRANCH_NAME = 1.* ]];then /usr/local/sonar-scanner/bin/sonar-scanner -X -Dsonar.projectKey=${Job_name} -Dsonar.projectName=${Job_name} -Dsonar.sourceEncoding=UTF-8 -Dsonar.projectVersion=${BUILD_ID} -Dsonar.sources=. ${binar} ${exclu}  ;fi 
         '''
          }
          }
        }
